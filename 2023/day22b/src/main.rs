use core::panic;
use std::{fs, usize, collections::HashMap, ops::Index};
...
fn main() {
    ...
    for brick in bricks {
        ...
        let mut dystroyed_bricks: Vec<u32> = Vec::new();
        dystroyed_bricks.push(brick.brick_number);
        let mut bricks_to_calculate: Vec<u32> = Vec::new();
        bricks_to_calculate.push(brick.brick_number);

        while bricks_to_calculate.len() > 0 {
            let current_brick_no = bricks_to_calculate.pop().unwrap();
            let mut bricks_on_top = get_supported_bricks(&bricks_map[&current_brick_no], &grid);

            for brick_no in &dystroyed_bricks {
                if bricks_on_top.contains(&brick_no) {
                    let index = bricks_on_top.iter().position(|x| x.clone() == brick_no.clone()).unwrap();
                    bricks_on_top.remove(index);
                }
            }

            println!("  Current brick: {}", brick.brick_number);
            println!("  Bricks on top: {:?}", bricks_on_top);
            
            if bricks_on_top.len() == 0 {
                dystroyed_bricks.push(current_brick_no);
            } else {

                // For each brick on top of the current one
                for brick_no in bricks_on_top {
                    //println!("  Checking: {}", brick_no);

                    let mut has_one_good_support = false;
                    // For each support of the brick
                    for support in get_supporting_bricks(&bricks_map[&brick_no], &grid) {
                        if dystroyed_bricks.contains(&support) == false {
                            has_one_good_support = true;
                        }
                    }

                    if !has_one_good_support {
                        println!("  No good supports: {}", brick_no);
                        bricks_to_calculate.push(brick_no);
                        dystroyed_bricks.push(brick_no);
                    }
        ...
        dystroyed_bricks.sort();
        dystroyed_bricks.dedup();
        //println!("  Total: {}", dystroyed_bricks.len() - 1);
        //println!("  Total: {:?}", dystroyed_bricks);
        total += dystroyed_bricks.len() - 1;
